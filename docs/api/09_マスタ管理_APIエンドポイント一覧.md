...
| 条件 | 動作 |
|---|---|
| 削除対象ユーザーが担当人材・案件を1件以上保持 | 削除不可。422を返す。エラーメッセージ：「担当中の案件が〇件、人材が〇件あるため削除できません。一覧画面から別の担当者へ変更してから再度実行してください。」（DB設計書 §7確定） |
| 担当人材・案件が0件 | 物理削除を実行する。`sub_user_id` の参照は `ON DELETE SET NULL` で自動的にNULLになる（DB設計書 §7確定） |
| 自分自身を削除しようとした場合 | 422を返す。エラーメッセージ：「自分自身のアカウントは削除できません」（TBD#2） |
| 対象ユーザーなし | 404 |
...
| # | 項目 | QA# | 理由 |
|---|------|-----|------|
| 1 | 自分自身の削除防止 | - | 管理者が自分自身を削除した場合の挙動が未定義。「削除不可・エラーを返す」方針で実装することを推奨するが確認が必要 |
| 2 | パスワードの要件 | - | パスワードの最低文字数（本書では暫定8文字）・大文字小文字数字記号の混在要件が未定義。QA #25「セキュリティは強めに対応」を踏まえて確認が必要 |
| 3 | 社内メールアドレスのドメイン | QA #19 | マスタ登録時・ログイン時の両方でドメインチェックを行う方針（二重チェック・暫定）だが、許容するドメイン（例：`@nexus.example.com`）が未確定。確定次第 `email` バリデーションに `ends_with` ルールを追加すること |
| 4 | 管理者が1人しかいない場合のロール変更・削除 | - | 最後の管理者ユーザーのロールを「一般」に変更したり削除したりすると管理者不在になる。ガード処理が必要かを確認すること |
