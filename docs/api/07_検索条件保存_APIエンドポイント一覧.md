...
### DELETE /saved-searches/{id}（#2）

#### アクセス可能ロール：管理者 / 一般営業（自分が保存した条件のみ削除可）

> 保存済み検索条件は専用の一覧・詳細画面を持たず、人材一覧（WF_03）・案件一覧（WF_06）の
> 「保存済み条件の管理」モーダルから削除する。

#### レスポンス

| 条件 | 動作 |
|---|---|
| 成功時 | 呼び出し元一覧画面へリダイレクトし SharedProps の `flash.success` を返す |
| 権限不足時（他ユーザーの保存条件を削除しようとした場合） | 前画面へリダイレクトし SharedProps の `flash.error` を返す |
| 対象データなし | 404 を返す |

> **【2026-08-30 修正】権限不足時のレスポンス方式を人材・案件・パイプライン削除と統一（issue #94）**
> **背景：** 従来は `authorize()` を素で呼んでおり、権限不足時に 403 がそのままフレームワークへ流れていた。
> PR #77（`ErrorPageResponder`）のマージ後は、この 403 が案内ページ（共通エラーページ）への
> 全画面遷移として描画されるようになり、一覧画面のモーダルから呼ばれる本操作の文脈から外れてしまう
> （人材・案件・パイプライン削除は元々 `try/catch` + `back()` + `flash.error` でこの遷移を避けている）。
> **理由：** 「一般ユーザーが権限のない削除を行おうとする」という操作類型は4箇所とも同一であり、
> 応答経路を分ける理由がないため、`EngineerController::destroy` と同じ `try/catch` +
> `back(fallback: route('dashboard'))` + `flash.error` 方式に統一した。
> **切り分け基準（今回整理）**：ミドルウェア（`EnsureUserIsAdmin` 等）によるルート単位のアクセス制御は、
> そもそもその画面へ遷移させるべきでないため 403（案内ページ）のままとする。一方、ユーザーがすでに
> 見えている画面上のアクション（削除ボタン等）に対する認可失敗は、画面から弾き出さず
> `back()` + `flash.error` で文脈を保つ。
