...
> **name 未入力時の扱い（画面側の仕様）**：name はAPI上は必須だが、画面の保存フォームでは任意入力とし、未入力の場合は現在の絞り込みタグ（例：「募集中 × フルリモート」）を組み合わせて画面側で自動生成し、常に非空文字列にしてから送信する。ただし絞り込み条件が0件かつ name も未入力の場合は、保存するものが実質無いため画面側で保存自体を不可とする（保存ボタンを無効化し、本APIへのリクエスト自体を送らない）。
