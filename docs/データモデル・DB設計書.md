...
パイプラインカードはマッチング結果経由でのみ生成される（手動追加不可。QA #43 にて確定）。1案件あたりに追加できるパイプラインの上限は**進行中（アクティブ）5件**（QA #50 にて確定。アプリ層で制御）。終了済みステータス（terminal：不成立・見送り等）は枠を消費しないため、過去に終了していれば同一案件へ再度追加できる（＝上限は「進行中のパイプライン数」で判定する。表示件数の「上位5件」とは別概念）。
