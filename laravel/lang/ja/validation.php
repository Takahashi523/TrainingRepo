...
return [
    ...
    */
    ...
    'custom' => [
        ...
        // engineer（人材登録フォーム）
        //
        // 機械的な required / max / date / boolean / in 等のメッセージは、
        // トップレベルのルールメッセージ（上部）＋ EngineerRequest::attributes() の
        // 日本語ラベルで十分自然な文面になるため custom からは削除した（#21 項目2）。
        // ここには「汎用テンプレートでは表現できない、フィールド固有の文面」だけを残す。
        // ----------------------------------------------------------------
        'name_kana' => [
            // regex: 汎用の「形式が正しくありません」ではカナ入力の意図が伝わらないため残す
            'regex' => 'カナで入力してください。',
        ],
        'birth_date' => [
            // before_or_equal: 汎用文面だと :date が "today" とそのまま表示され不自然なため残す
            'before_or_equal' => '生年月日は今日以前の日付を入力してください。',
        ],
        'skills' => [
            // required: 「1件以上追加」という配列固有の操作案内のため残す
            'required' => 'スキルを1件以上追加してください。',
        ],
        'skills.*.label' => [
            // required_with: 詳細入力時にラベルを促す固有の案内のため残す
        ...
        'rate_min' => [
            ...
            'min' => '正しい金額を入力してください。',
            'lte' => '単価は「下限 ≤ 上限」で入力してください。',
        ],
        'rate_max' => [
            'required' => '単価上限も入力してください。',
            'integer' => '正しい金額を入力してください。',
            'min' => '正しい金額を入力してください。',
            'gte' => '単価は「下限 ≤ 上限」で入力してください。',
        ],
        'commercial_flow' => [
            'in' => '商流を選択してください。',
        ],
