...
class TestFetchActiveProjects:
    ...
    def _make_skill_row(self, project_id, skill_type, label, detail=None):
        return _row(
            project_id=project_id, skill_type=skill_type, label=label, detail=detail
        )
    ...
    def test_returns_all_open_projects_when_no_filter(self):
        ...
        assert results[0].skills[0] == ProjectSkill(
            skill_type="required", label="Java", detail="Spring Boot"
        )
...
class TestCascadeSortHelpers:
    ...
    def test_work_style_match_onsite(self):
        engineer = _make_engineer(
            work_style_onsite=1, work_style_hybrid=0, work_style_remote=0
        )
    ...
    def test_work_style_match_hybrid(self):
        engineer = _make_engineer(
            work_style_onsite=0, work_style_hybrid=1, work_style_remote=0
        )
    ...
    def test_work_style_match_remote(self):
        engineer = _make_engineer(
            work_style_onsite=0, work_style_hybrid=0, work_style_remote=1
        )
...
class TestCascadeSort:
    ...
    def test_higher_proc_overlap_ranks_first(self):
        ...
        engineer = _make_engineer(
            proc_requirements=1, proc_development=1, proc_testing=1
        )
        p_low = _make_project(
            1, proc_requirements=1, proc_development=0, proc_testing=0
        )  # 重複1
        p_high = _make_project(
            2, proc_requirements=1, proc_development=1, proc_testing=1
        )  # 重複3
    ...
    def test_rate_in_range_preferred_when_proc_tied(self):
        ...
        engineer = _make_engineer(
            desired_rate=80, proc_requirements=0, proc_development=0
        )
...
class TestCalculateMatching:
    ...
    def test_returns_top5_sorted_by_score(self, mocker):
        ...
        """
        engineer = _make_engineer()
        projects = [_make_project(i) for i in range(1, 7)]  # 6件
        ...
        # ★ matching_service.py が `from ... import invoke_matching` しているため、
        # パッチは呼び出し元である matching_service 側の名前に対して行う
        ...
        assert result.matches[4].match_score == 55  # 5位（6位の40は除外される）
        assert mock_invoke.call_count == 6  # 6件全てにAI呼び出し（30件以下は全件評価）
    ...
    def test_excludes_pipeline_registered_projects(self, mocker):
        ...
        # ★ 呼び出し元（matching_service）の名前空間をパッチ
        mock_invoke = mocker.patch(
            "app.services.matching_service.invoke_matching",
    ...
    def test_cascade_sort_applied_when_over_30_candidates(self, mocker):
        """候補が31件以上の場合、カスケードソートで上位30件に絞ってから AI 呼び出しされること
        （スコアリングロジック設計書 v0.6 §3.4 Step 3.6 準拠。AIは候補ごとに個別呼び出しするため、
        1プロンプトへの一括投入トークン制限の対象にはならない）。
        """
        engineer = _make_engineer()
        ...
        mock_invoke = mocker.patch(
            "app.services.matching_service.invoke_matching",
            side_effect=[_make_ai_result(50 + (i % 30)) for i in range(30)],
        )

        result = calculate_matching(MagicMock(), engineer_id=1, project_ids=None)

        assert mock_invoke.call_count == 30  # 35件 → カスケードソートで30件に絞込
        assert len(result.matches) == 5  # レスポンスとして返るのは常に上位5件
    ...
    def test_empty_candidates_raises_no_active_candidate_error(self, mocker):
        ...
        mock_invoke = mocker.patch("app.services.matching_service.invoke_matching")
    ...
    def test_ai_comment_and_ai_missing_propagate_correctly_per_candidate(self, mocker):
        """推薦理由(ai_comment)・不足条件説明(ai_missing)が、
        取り違えられることなく各案件（project_id）に正しく紐付いて
        最終結果（MatchCandidate）まで伝播すること。
        """
        engineer = _make_engineer()
        projects = [_make_project(1), _make_project(2), _make_project(3)]
        ...
        # 案件ごとに全く異なる ai_comment / ai_missing を用意し、
        # 「どの案件の結果か」が最後まで正しく対応しているかを検証できるようにする
        ai_results = [
            MatchingAIResult(
                match_score=90, match_rank="A",
                ai_score_reason="案件1の理由", ai_comment="案件1向けの推薦コメント", ai_missing="案件1の不足条件",
            ),
            MatchingAIResult(
                match_score=70, match_rank="B",
                ai_score_reason="案件2の理由", ai_comment="案件2向けの推薦コメント", ai_missing="案件2の不足条件",
            ),
            MatchingAIResult(
                match_score=50, match_rank="C",
                ai_score_reason="案件3の理由", ai_comment="案件3向けの推薦コメント", ai_missing="案件3の不足条件",
            ),
        ]
        mocker.patch(
            "app.services.matching_service.invoke_matching",
        ...
        matches_by_project = {m.project_id: m for m in result.matches}
        assert matches_by_project[1].ai_comment == "案件1向けの推薦コメント"
        assert matches_by_project[1].ai_missing == "案件1の不足条件"
        assert matches_by_project[2].ai_comment == "案件2向けの推薦コメント"
        assert matches_by_project[2].ai_missing == "案件2の不足条件"
        assert matches_by_project[3].ai_comment == "案件3向けの推薦コメント"
        assert matches_by_project[3].ai_missing == "案件3の不足条件"
...
class TestGenerateProfileSummary:
    ...
    def test_returns_summary_and_generated_at(self, mocker):
        ...
        # ★ 呼び出し元（matching_service）の名前空間をパッチ
        mocker.patch(
            "app.services.matching_service.invoke_profile_summary",
    ...
    def test_bedrock_error_propagates(self, mocker):
        """Bedrock 呼び出し失敗時、BedrockError が伝播すること。"""
